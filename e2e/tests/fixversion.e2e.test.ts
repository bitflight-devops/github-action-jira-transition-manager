...
describe('FixVersion E2E Tests', () => {
  ...
  describe('Project and Version Setup', () => {
    it('should have the test project', async () => {
  ...
  describe('Version Idempotency', () => {
    it('should handle existing version without error', async () => {
  ...
  describe('Issue FixVersion Management', () => {
    it('should create issue with fixVersion', async () => {
    ...
    it('should update issue fixVersion', async () => {
      ...
      // Verify update
      const updatedIssue = await client.getIssue(issue.key, [
        'fixVersions',
        'summary',
      ]);
    ...
    it('should find issues with specific fixVersion', async () => {
      ...
      const results = await client.searchIssues(
        `project = ${projectKey} AND fixVersion = "${targetVersion}"`,
        ['key', 'summary', 'fixVersions'],
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should list all versions for project', async () => {
      const versions = await client.listProjectVersions(projectKey);
      ...
      const hasInitial = versions.some(
        (v) => v.name === config.test.initialVersion,
      );
      expect(hasInitial).toBe(true);
    }, config.timeouts.testTimeout);

    it('should handle empty fixVersions array', async () => {
      const issue = await client.createIssue(
        projectKey,
        'Issue without fixVersion',
        config.test.issueType,
      );
